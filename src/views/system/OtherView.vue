...
const projectCacheKey = 'other_projects_cache'
...
const loadCachedProjects = () => {
  try {
    ...
    return filterProjectsForCurrentUser(parsed, getProjectListUserContext(ADMIN_EMAILS), 'other')
  } catch {
    return []
  }
}
...
onMounted(() => {
  ...
  const { data } = supabase.auth.onAuthStateChange((event, session) => {
    if (session?.user && route.path === '/other') {
      loadProjects()
    }
...
const handleVisibilityChange = () => {
  if (document.visibilityState === 'visible' && route.path === '/other') {
    loadProjects()
  }
}
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const projectMembersSelect = getProjectMembersSelect(userContext, 'other')
        ...
        const visibleProjects = filterProjectsForCurrentUser(data || [], userContext, 'other')
        ...
        const shouldRetryEmptyResult =
          mappedProjects.length === 0 && attempt < maxAttempts && route.path === '/other'
      ...
      <v-container fluid class="projects-container pa-5">
        ...
        <div class="controls-section">
          ...
          <div class="action-buttons">
            ...
            <RouterLink v-if="canAddProject" to="/other/new" class="add-project-btn"
