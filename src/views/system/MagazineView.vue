...
const projectCacheKey = 'magazine_projects_cache'
...
const loadCachedProjects = () => {
  try {
    ...
    return filterProjectsForCurrentUser(parsed, getProjectListUserContext(ADMIN_EMAILS), 'magazine')
  } catch {
    return []
  }
}
...
onMounted(() => {
  ...
  const { data } = supabase.auth.onAuthStateChange((event, session) => {
    if (session?.user && route.path === '/magazine') {
      loadProjects()
    }
...
const handleVisibilityChange = () => {
  if (document.visibilityState === 'visible' && route.path === '/magazine') {
    loadProjects()
  }
}
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (shouldScopeProjectsByType(userContext, 'magazine')) {
          query = query.eq('project_type', 'magazine')
        }

        query = applyProjectListVisibility(query, userContext, 'magazine')

        const { data, error } = await query
        if (error) throw error

        const visibleProjects = filterProjectsForCurrentUser(data || [], userContext, 'magazine')
        ...
        const shouldRetryEmptyResult =
          mappedProjects.length === 0 && attempt < maxAttempts && route.path === '/magazine'
...
const handleView = (projectId) => {
  ...
  const actualType = project.project_type || 'magazine'
...
const saveEdit = async () => {
  ...
  try {
    ...
    // Save a history snapshot for list-based edits so changes appear in Project History.
    try {
...
<template>
  <v-app class="magazine-page">
    ...
    <!-- Edit Project Modal - IMPROVED -->
    <v-dialog v-model="showEditDialog" max-width="550px" persistent>
      <v-card class="edit-dialog-card mb-5">
