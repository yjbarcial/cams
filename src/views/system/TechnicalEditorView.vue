...
const getNextStatus = (action) => {
  ...
  if (action === 'return') return 'Returned by Technical Editor'
    ...
    if (queryType) {
      ...
      if (queryType === 'other' || queryType === 'social-media') {
        ...
        if (actualStorage) {
          ...
          if (foundProject) {
            console.log('✅ Project found in', actualStorage.type, ':', foundProject)
            project.value = {
              ...foundProject,
              id: String(projectId),
...
const getStatusColor = (status) => {
  const statusColors = {
    ...
    'Returned by Technical Editor': 'warning',
