...
const projectCacheKey = 'folio_projects_cache'
...
const loadCachedProjects = () => {
  try {
    ...
    return filterProjectsForCurrentUser(parsed, getProjectListUserContext(ADMIN_EMAILS), 'folio')
  } catch {
    return []
  }
}
...
onMounted(() => {
  ...
  const { data } = supabase.auth.onAuthStateChange((event, session) => {
    if (session?.user && route.path === '/folio') {
      loadProjects()
    }
...
const handleVisibilityChange = () => {
  if (document.visibilityState === 'visible' && route.path === '/folio') {
    loadProjects()
  }
}
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const projectMembersSelect = getProjectMembersSelect(userContext, 'folio')
        ...
        const visibleProjects = filterProjectsForCurrentUser(data || [], userContext, 'folio')
        ...
        const shouldRetryEmptyResult =
          mappedProjects.length === 0 && attempt < maxAttempts && route.path === '/folio'
      ...
      <v-container fluid class="projects-container pa-5">
        ...
        <div class="controls-section">
          ...
          <div class="action-buttons">
            ...
            <RouterLink v-if="canAddProject" to="/folio/new" class="add-project-btn"
