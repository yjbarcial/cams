...
const handleView = (projectId) => {
  ...
  if (project.status === 'Draft' || project.status === 'Returned by Section Head') {
    router.push({ path: `/project/${projectId}`, query: { type: 'folio' } })
  } else if (
    project.status === 'To Section Head' ||
    project.status === 'Returned by Technical Editor'
  ) {
    router.push({ path: `/section-head/${projectId}`, query: { type: 'folio' } })
  } else if (project.status === 'To Technical Editor') {
    router.push({ path: `/technical-editor/${projectId}`, query: { type: 'folio' } })
  } else if (
    ...
    router.push({ path: `/editor-in-chief/${projectId}`, query: { type: 'folio' } })
  } else if (project.status === 'To Chief Adviser' || project.status === 'Adviser Review') {
    router.push({ path: `/chief-adviser/${projectId}`, query: { type: 'folio' } })
